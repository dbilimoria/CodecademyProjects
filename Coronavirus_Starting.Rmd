...
dead <- read_csv("corona_deaths.csv")
...
northern_cases <- confirmed %>%
  filter(Lat >0) %>%
  select(`3/22/20`) %>%
  sum()

print(northern_cases)
...
print(australia_cases)

# Display percentages
world_cases <- confirmed %>%
  select(`3/22/20`) %>%
  sum()
world_cases
aus_percentage <- australia_cases / world_cases * 100
aus_percentage
```
...
head(confirmed_by_country, 10)

# Filter the grouped data set for Australia
double_checking <- confirmed_by_country %>%
  filter(`Country/Region` == 'Australia')
...
us_cases <- confirmed_by_country %>%
  filter(`Country/Region` == 'US') %>%
  select(`3/22/20`)

us_recovered <- recovered_by_country %>%
  filter(`Country/Region` == 'US') %>%
  select(`3/22/20`)

us_recovered_percentage <- us_recovered / us_cases 

print(us_cases)
us_recovered
us_recovered_percentage
```
...
max_cases_us <- confirmed_by_country %>%
  filter(`Country/Region` == 'US') %>%
...
confirmed_by_country_transposed <- confirmed_by_country %>%
  t() %>%
  as.data.frame()
head(confirmed_by_country_transposed)
...
```{r}
library(janitor)
confirmed_by_country_transposed <- confirmed_by_country_transposed %>%
...
confirmed_by_country_transposed <- apply(confirmed_by_country_transposed, 2, as.numeric) %>%
  as.data.frame()
...
max_cases_us_transposed <- confirmed_by_country_transposed %>%
  select(US) %>%
  max()

max_cases_us_transposed
...
```{r}
# Add the date column
nrow_transposed <- nrow(confirmed_by_country_transposed)
nrow_transposed
confirmed_by_country_transposed <- confirmed_by_country_transposed %>%
  mutate(date = c(1:61))
head(select(confirmed_by_country_transposed, date))
...
library(ggplot2)
cases_italy <- ggplot(data = confirmed_by_country_transposed, aes(x=date, y=Italy)) + geom_line()
...
cases_italy <- cases_italy + labs(x="Number of Days Since Jan 22, 2020", y="Number of Confirmed Cases", title="Coronavirus Cases in Italy")
cases_italy
```
