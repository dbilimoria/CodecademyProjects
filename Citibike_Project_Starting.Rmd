...
# Create a heatmap
starting_locations <- ggplot(data=all_data, aes(x = start.station.longitude, y=start.station.latitude)) + geom_bin2d(binwidth = c(0.001, 0.001))
starting_locations
...
average_speed_by_age_plot <- ggplot(data = average_speed_by_age, aes(x=age, y=mean_speed)) + geom_line()
...
less_than_80_plot <- average_speed_by_age_plot + coord_cartesian(xlim = c(16,80), ylim = c(0,3))

less_than_80_plot

less_than_80_filtered <- average_speed_by_age %>%
  filter(age < 80)

less_than_80_filtered_plot <- ggplot(data = less_than_80_filtered, aes(x=age, y=mean_speed)) + geom_line()
less_than_80_filtered_plot
...
less_than_80_filtered_plot <- less_than_80_filtered_plot + labs(title = "Average Speed of CitiBike Users (Jan 2020) in NY", x = "Age", y= "Speed in Meters Per Second")
...
age_and_gender_plot <- ggplot(data = average_speed_by_age_and_gender, aes(x = age, y = mean_speed)) + geom_line(aes(color=gender))
...
age_and_gender_plot <- ggplot(data = average_speed_by_age_and_gender, aes(x = age, y = mean_speed)) + geom_line(aes(color=gender)) + labs(title = "Average Speed of CitiBike Users (NY, Jan 2020)", x = "Age", y= "Speed in Meters Per Second") + scale_color_discrete(name = "Gender", labels = c("Male Identifying", "Female Identifying"))
...
age_count_by_gender <- all_data %>%
  group_by(age, gender)%>%
  tally()

age_count_by_gender <- age_count_by_gender %>%
  mutate(gender = as.factor(gender)) %>%
  filter(gender == 1|gender == 2) %>%
  filter(age < 100)
...
age_count_plot <- ggplot(data=age_count_by_gender, aes(x = age, y = n, fill = gender)) + geom_col() + labs(title= "Number of CitiBike Users NY, Jan 20020", x = "Age", y= "Count")+ scale_fill_discrete(name = "Gender", labels = c("Male Identifying", "Female Identifying"))
